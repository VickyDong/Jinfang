...
public class DVP_MODBUS_RTU extends CSlaveProtocol {
	...
	private String s_deviceAddr;                                        // 设备地址
	...
	private byte m_ReadCoilStatus = (byte) 0x01;                        // 读继电器输出
	private byte m_ReadInputStatus = (byte) 0x02;                       // 读开关量输入
	private byte m_ReadHoldingRegister = (byte) 0x03;                   // 读保持寄存器

	private byte m_ForceSingleCoil = (byte) 0x05;                       // 写单线圈

	private byte m_PresetMultipleRegister=(byte)0x10;                   // 写多寄存器
	...
	public DVP_MODBUS_RTU(int recordNo) throws IOException {
		m_recordNo = recordNo;
	...
	public byte[] FormCommandPackage(String plc_id,String sensor_id,byte[] value,byte command) throws Exception{
		...
		if(command == m_ForceSingleCoil){                    //写开关量
			buffer = new byte[8];
		...
		}else if (command == m_PresetMultipleRegister){      //写模拟量			
	...
	public byte[] Sort(String plc_id,String sensor_id,byte[] value) throws Exception{
		...
		String sql = "select * from address_division where Device_ID='"+ s_deviceAddr+ "' and Plc_ID='"+ plc_id+ "' and Start_Address='"+ sensor_id + "' ";
	...
	public byte[] SendCommand(String plc_id, String sensor_id, float value,int downtype) throws Exception { // 下发遥控指令
		System.out.println("********** DVP_MODUSBUS_RTU 处理命令");
		if (downtype == 0) { 
			d_commandType = m_ForceSingleCoil;         //开关量
			d_value = new byte[2];
			int x =(int) value;
			...
			d_commandType = m_PresetMultipleRegister;
	...
	public byte[] SendProcess() throws Exception { // 处理发送请求
		System.out.println("********** DVP_MODUSBUS_RTU 发送请求");
	...
	public byte[] FormAnalogPackage(String plc, String table)  throws Exception {   // 对查询模拟量的打包
		...
		m_sendType = m_ReadHoldingRegister;
		...
		if(flag){                                  //取地址
			...
			conn.close();
			if(title.size()>0){
				m_sensor_address  = GetAnalogAddress(title, count);                  //存放的是实际地址
		...
		if (m_sensor_count > 0) {                     // 有纪录在表中
			...
			String address = m_sensor_address.get(m_sensor_start);
			
			m_beginAddr[0] = HexStringToByte(address.substring(0, 2));            // 纪录起始地址
			m_beginAddr[1] = HexStringToByte(address.substring(2));
			
			m_address = ReCovertAddress(address);                              //到起始地址,如D0
			...
			if (m_pollTable <= m_databaseTable.length - 1
					&& m_pollPlc < m_plcId.length - 1) { // 不在循环边缘，内层加1即可
				m_pollPlc++;
				m_sensor_start = 0;
				flag = true;
			} else if (m_pollTable < m_databaseTable.length - 1
	...
	public byte[] FormDigitalPackage(String plc, String table) // 对查询数字量进行打包,其中，8个data形成一个字节数据
		...
		if(flag){                                             //取地址
			...
			conn.close();
			if(origin.size()>0){
				m_sensor_address  = GetDigitalAddress(origin);
				ContentComparator comp = new ContentComparator();    
				Collections.sort(m_sensor_address,comp);
				flag = false;
			}else{
		...
		if (m_sensor_address!=null&&( m_sensor_address.size()>0 ) ) {
			...
			if (m_sensor_address.get(m_sensor_start).substring(0, 2).equals("04"))
				m_sendType = m_ReadInputStatus;
			else
				m_sendType = m_ReadCoilStatus;
			...
			byte[] datacount = Calculate(m_send_digital_count); // 执行打包的过程,依次为PLC-ID,发送类型,起始地址高位\低位,个数高位\低位,以及CRC校验
			...
			buffer[6] = crc[0];
			buffer[7] = crc[1];
			if(m_send_digital_count %8 == 0)
				m_shouldLength = m_send_digital_count+5;
			else
				m_shouldLength = m_send_digital_count/8 + 6;
		} else {
			if (m_pollTable <= m_databaseTable.length - 1
					&& m_pollPlc < m_plcId.length - 1) {// 不在循环边缘，内层加1即可
				m_pollPlc++;
	...
	public int ReceiveProcess(byte[] buffer) throws Exception {// 处理数据上报报文，参数为剥离了IHDC和H700协议之后的数据
		...
		System.out.println("**********MODBUS处理");
		System.out.println("**********DVP_RTU ");
			...
			if (buffer.length == m_shouldLength) {                     //收到的数据包长度是否正确
				...
				if (d_receiveReply) { // 下发命令包的响应包
					...
					if (recv[0] == DecimalStringToByte(plcId)                                   //检查头\尾,以及CRC校验是否正确
						...
						System.out.print("**********收到数据包长度是："+buffer.length+",应该长度是："+m_shouldLength+",通过头 尾,以及CRC校验,内容是：");
						for (int i = 0; i < recv.length; i++)
							System.out.print(+recv[i] + "  ");
						System.out.println();
						MsgReceiverFactory.receiver.receive(s_deviceAddr,recv, recv.length);
						...
						if (function == m_ReadCoilStatus) {
							...
							for (int i = 0; i < dataBuffer.length; i++) {            //上报的数据是开关量
								for (int j = 0; j < 8; j++) {
									...
									System.out.print("**********设备地址: "+s_deviceAddr+" PLC编号: "+slave_address+"  寄存器地址:"+m_address+"  值为:"+data+" 状态为:"+status);
									System.out.println();
//									components.UpDigitalData(s_deviceAddr,slave_address, m_address, data, status);
									...
									if (str.equalsIgnoreCase("X")|| str.equalsIgnoreCase("Y")) {
										...
										m_address = str.concat(Integer.toOctalString(number).toUpperCase());
									} else {
										int number = Integer.parseInt(m_address.substring(1), 10) + 1;
										m_address = str.concat(Integer.toString(number, 10).toUpperCase());
									}
							...
							for (int i = 0; i < dataBuffer.length; i++) {
								for (int j = 0; j < 8; j++) {
									...
									status = 1;
									System.out.print("**********上传数据是: ");
									System.out.print("**********设备地址: "+s_deviceAddr+" PLC编号: "+slave_address+"  寄存器地址:"+m_address+" 值为:"+data+"  状态是:"+status);
									System.out.println();
//									components.UpDigitalData(s_deviceAddr,slave_address, m_address, data,status);
						...
						} else if (function == m_ReadHoldingRegister) {                   // 模拟量数据,需要进行计算,得出其值
							...
							while (notEnd) {
								...
								ResultSet rs = stmt.executeQuery(sql);
								if (rs.next()) {
									System.out.print("**********数据位是: "+symbol+" 地址是: "+m_address+" 已查到记录,");									
									...
									System.out.println();
//									components.UpAnalogData(s_deviceAddr,slave_address, m_address, data, status);
									
									if(word_count==1){
										symbol = symbol + 2;
										int number = Integer.parseInt(m_address.substring(1), 10)+1;
										m_address = m_address.substring(0, 1).concat(Integer.toString(number, 10).toUpperCase());
									
									}else if(word_count==2){
										symbol = symbol + 4;
										int number = Integer.parseInt(m_address.substring(1), 10)+2;
										m_address = m_address.substring(0, 1).concat(Integer.toString(number, 10).toUpperCase());
									...
									System.out.print("**********数据位是: "+symbol+" 地址是: "+m_address+" 未查到记录");
									System.out.println();
									symbol = symbol + 2;
									int number = Integer.parseInt(m_address.substring(1), 10)+1;
									m_address = m_address.substring(0, 1).concat(Integer.toString(number, 10).toUpperCase());
							...
							if (m_send_analog_count == 100) {
								if(m_sensor_end == m_sensor_count - 1){
									int beginNumber = Integer.parseInt(m_sensor_address.get(m_sensor_end), 16);
									int endNumber = Integer.parseInt(m_sensor_address.get(m_sensor_start),16);
				...
				System.out.print("**********收到数据包长度是："+buffer.length+",应该长度是："+m_shouldLength+",包的内容是:");
				byte[] temp = Arrays.copyOfRange(buffer, 0, buffer.length);				
				for (int i = 0; i < temp.length; i++)
					System.out.print(temp[i] + "  ");
				System.out.println();
			...
			m_count++;
			if(m_count>15){
				if(m_sendType == m_ReadCoilStatus || m_sendType == m_ReadInputStatus ){
					int status = 0;
...
//						components.UpdateStatus(s_deviceAddr,plcId, m_address,status);
				...
				}else if(m_sendType == m_ReadHoldingRegister){
					int status = 0;
...
//						components.UpdateStatus(s_deviceAddr,plcId, m_address,status);
	...
	public void QueryPLC(int m_recordNo) throws Exception {// 查询与此主机相关的plc机编号，数据库中存放的是varchar类型，因此需要转化为byte类型
		...
		Connection conn = dc.getmysql();
		Statement stmt = conn.createStatement();
		String querySql = "select * from deviceinfo where record_number='"+ m_recordNo + "'";
		ResultSet rs = stmt.executeQuery(querySql);
		if (rs.next()) {
			temp = rs.getString("plc_id"); // 查询与本机相关的plc机编号
			s_deviceAddr = rs.getString("device_id"); // 查询与本机相关的设备号
		} else
			return;
