...
public class DVP_MODBUS_ASC extends CSlaveProtocol {
	...
	public int ReceiveProcess(byte[] buffer,int send) throws Exception {  // 接收处理

		System.out.println("**********MODBUS_ASC处理");
		System.out.println("**********DVP_ASC");
		if (buffer != null) {                                            
			a_count++;
...
//							a_waitCount = 0;
			...
			//} else {
				...
				for (int i = 0; i < temp.length; i++) {
					...
					if(s.length()==1) bufferOutput.append("0"+s+" ");
					else
						bufferOutput.append(s+" ");
				}
				int command_id = send%9+1;
				if(command_id==1){
					int serial_number = Integer.parseInt(bufferOutput.substring(15,17),16)*65536+Integer.parseInt(bufferOutput.substring(18,20),16)*256
							+Integer.parseInt(bufferOutput.substring(21,23),16);
					System.out.println("序列号： "+serial_number);
					System.out.println("ALM1: "+String.format("%08d",Integer.parseInt(Integer.toBinaryString(Integer.valueOf(bufferOutput.substring(24,26))))));
					System.out.println("ALM2: "+String.format("%08d",Integer.parseInt(Integer.toBinaryString(Integer.valueOf(bufferOutput.substring(27,29))))));
					System.out.println("ALM3: "+String.format("%08d",Integer.parseInt(Integer.toBinaryString(Integer.valueOf(bufferOutput.substring(30,32))))));
					double temprature = Integer.parseInt(bufferOutput.substring(33,38).replaceAll(" ",""),16)/10.0;
					System.out.println("模块温度（°C）：  "+temprature);
					if(bufferOutput.substring(39,41).equals("00"))
						System.out.println("工作模式： APC方式");
					else
						System.out.println("工作模式： ACC方式");
					if(bufferOutput.substring(42,44).equals("00"))
						System.out.println("Op_Para： ACC方式");
					else
						System.out.println("Op_Para： APC方式");

					double Pin = Integer.parseInt((bufferOutput.substring(45,50).replaceAll(" ","")),16)/10.0-70;
					double Pout = Integer.parseInt((bufferOutput.substring(51,56).replaceAll(" ","")),16)/10.0-70;
					double Pin_th = Integer.parseInt((bufferOutput.substring(57,62).replaceAll(" ","")),16)/10.0-70;
					double Pout_th = Integer.parseInt((bufferOutput.substring(63,68).replaceAll(" ","")),16)/10.0-70;
					System.out.println("输入光功率（dbm）： "+String.format("%.2f",Pin));
					System.out.println("输出光功率（dbm）： "+String.format("%.2f",Pout));
					System.out.println("输入光功率门限（dbm）： "+String.format("%.2f",Pin_th));
					System.out.println("输出光功率门限（dbm）： "+String.format("%.2f",Pout_th));

					double P1_lop = Integer.parseInt((bufferOutput.substring(69,74).replaceAll(" ","")),16)/10.0;
					double P1_power = Integer.parseInt((bufferOutput.substring(75,80).replaceAll(" ","")),16)/10.0;
					double P1_Tchip = Integer.parseInt((bufferOutput.substring(81,86).replaceAll(" ","")),16)/10.0;
					double P1_cooler = Integer.parseInt((bufferOutput.substring(87,92).replaceAll(" ","")),16)/10.0-3000;
					System.out.println("泵浦1工作电流（mA）： "+P1_lop);
					System.out.println("泵浦1泵浦功率（mW）： "+P1_power);
					System.out.println("泵浦1管芯温度（°C）： "+P1_Tchip);
					System.out.println("泵浦1制冷电流（mA）： "+P1_cooler);

					double P2_lop = Integer.parseInt((bufferOutput.substring(93,98).replaceAll(" ","")),16)/10.0;
					double P2_power = Integer.parseInt((bufferOutput.substring(99,104).replaceAll(" ","")),16)/10.0;
					double P2_Tchip = Integer.parseInt((bufferOutput.substring(105,110).replaceAll(" ","")),16)/10.0;
					double P2_cooler;
					if(P2_lop==0&&P2_power==0&&P2_Tchip==0)
						P2_cooler = 0;
					else
						P2_cooler = Integer.parseInt((bufferOutput.substring(111,116).replaceAll(" ","")),16)/10.0-3000;
					System.out.println("泵浦2工作电流（mA）： "+P2_lop);
					System.out.println("泵浦2泵浦功率（mW）： "+P2_power);
					System.out.println("泵浦2管芯温度（°C）： "+P2_Tchip);
					System.out.println("泵浦2制冷电流（mA）： "+P2_cooler);
					...
					System.out.println("ALM1: "+String.format("%08d",Integer.parseInt(Integer.toBinaryString(Integer.valueOf(bufferOutput.substring(15,17))))));
					System.out.println("ALM2: "+String.format("%08d",Integer.parseInt(Integer.toBinaryString(Integer.valueOf(bufferOutput.substring(18,20))))));
					System.out.println("ALM3: "+String.format("%08d",Integer.parseInt(Integer.toBinaryString(Integer.valueOf(bufferOutput.substring(21,23))))));
				}
				else if(command_id==4){
					double temprature = Integer.parseInt(bufferOutput.substring(15,20).replaceAll(" ",""),16)/10.0;
					System.out.println("模块温度（°C）：  "+temprature);
				}
				else if(command_id==5){
					int num = Integer.parseInt((bufferOutput.substring(15,17)),16);
					System.out.println("泵浦数目： "+num);
				}
				else if(command_id==6){
					double P1_lop = Integer.parseInt((bufferOutput.substring(15,20).replaceAll(" ","")),16)/10.0;
					double P1_power = Integer.parseInt((bufferOutput.substring(21,26).replaceAll(" ","")),16)/10.0;
					double P1_Tchip = Integer.parseInt((bufferOutput.substring(27,32).replaceAll(" ","")),16)/10.0;
					double P1_cooler = Integer.parseInt((bufferOutput.substring(33,38).replaceAll(" ","")),16)/10.0-3000;
					System.out.println("泵浦1工作电流（mA）： "+P1_lop);
					System.out.println("泵浦1泵浦功率（mW）： "+P1_power);
					System.out.println("泵浦1管芯温度（°C）： "+P1_Tchip);
					System.out.println("泵浦1制冷电流（mA）： "+P1_cooler);
					...
					double P2_lop = Integer.parseInt((bufferOutput.substring(15,20).replaceAll(" ","")),16)/10.0;
					double P2_power = Integer.parseInt((bufferOutput.substring(21,26).replaceAll(" ","")),16)/10.0;
					double P2_Tchip = Integer.parseInt((bufferOutput.substring(27,32).replaceAll(" ","")),16)/10.0;
					double P2_cooler;
					if(P2_lop==0&&P2_power==0&&P2_Tchip==0)
						P2_cooler = 0;
					else
						P2_cooler = Integer.parseInt((bufferOutput.substring(33,38).replaceAll(" ","")),16)/10.0-3000;
					System.out.println("泵浦2工作电流（mA）： "+P2_lop);
					System.out.println("泵浦2泵浦功率（mW）： "+P2_power);
					System.out.println("泵浦2管芯温度（°C）： "+P2_Tchip);
					System.out.println("泵浦2制冷电流（mA）： "+P2_cooler);
					...
					double Pin = Integer.parseInt((bufferOutput.substring(15,20).replaceAll(" ","")),16)/10.0-70;
					double Pout = Integer.parseInt((bufferOutput.substring(21,26).replaceAll(" ","")),16)/10.0-70;
					double Pin_th = Integer.parseInt((bufferOutput.substring(27,32).replaceAll(" ","")),16)/10.0-70;
					double Pout_th = Integer.parseInt((bufferOutput.substring(33,38).replaceAll(" ","")),16)/10.0-70;
					System.out.println("输入光功率（dbm）： "+String.format("%.2f",Pin));
					System.out.println("输出光功率（dbm）： "+String.format("%.2f",Pout));
					System.out.println("输入光功率门限（dbm）： "+String.format("%.2f",Pin_th));
					System.out.println("输出光功率门限（dbm）： "+String.format("%.2f",Pout_th));
					...
					if(bufferOutput.substring(15,17).equals("00"))
						System.out.println("工作模式： APC方式");
					else
						System.out.println("工作模式： ACC方式");
					if(bufferOutput.substring(18,20).equals("00"))
						System.out.println("Op_Para: ACC方式");
					else
						System.out.println("Op_Para: APC方式");
				}
