...
public class CIHDC extends CDeviceProtocol {
	...
	public void ReceiveProcess(byte[] i_tempBuffer, int i_tempLength,int send)
			throws Exception {
		// 接收处理
		...
		if (i_tempBuffer[0] == header && i_tempBuffer[1] == login) {// 登录包，则做登录响应
			...
			if (i_connectionProtocol.equals("UDP")) {
				...
				if (pl != null){
					pl.ReceiveProcess(buffer,send);// 调用MODBUS协议来处理
					canSend = true;
				}else
					return;				
				...
				if (i_tempBuffer[i_tempLength - 1] == header) {
					byte[] buffer = Arrays.copyOfRange(i_tempBuffer, 15,
							i_tempLength - 1);
					if (pl != null)
						pl.ReceiveProcess(buffer,send);

				} else {
					byte[] buffer = Arrays.copyOfRange(i_tempBuffer, 15,
							i_tempLength);
					if (pl != null)
						pl.ReceiveProcess(buffer,send);// 调用MODBUS协议处理
				}
			}
		}
	...
	public void MainProcess(byte[] buffer, int length, int send) throws Exception {
		...
		if (length > 1) { // 收到数据包,做相关处理
			...
			ReceiveProcess(temp, length, send);
			if (canSend) {
				SendProcess();
			}
