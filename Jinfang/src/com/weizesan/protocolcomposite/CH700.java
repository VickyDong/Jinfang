...
//	private byte wrongdsc=(byte)0x04;//收到来自dsc的无效数据包
	...
	public void ReceiveProcess(byte[] buffer, int length, int send) throws Exception {
		...
		if (buffer[0] == header && buffer[length - 1] == tail) {     //包头和包尾都是固定的值,便于判断
			...
			if (buffer[1] == login) {// 登录包，则做登录响应,或者是发来的心跳包
				...
				if (pl != null){
					int symbol = -1;
					try {
						symbol = pl.ReceiveProcess(c_dataBuffer,send); // 调用MODBUS协议处理
					} catch (Exception e) {
						e.printStackTrace();
					}
					if(symbol == 2){
	...
	public void MainProcess(byte[] buffer,int length,int send) throws Exception   //主处理
	{
