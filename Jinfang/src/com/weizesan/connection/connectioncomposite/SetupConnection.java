...
public class SetupConnection {
	...
	public void init() throws Exception {
		...
		String sqlserver = "select * from deviceinfo where sqlserver_ipaddress='"+ "127.0.1.1" + "'";
	...
	class HandlerTcp implements Runnable {
		...
		public void run() {
			...
			System.out.println("**********client: " + clientCount +"  IP: "+dip+" port: "+socket.getPort());
			...
			try {
				while (true) {
					...
					if (sendRequest) {
						...
						DataBaseComposite dc = new DataBaseComposite();
						Connection conn = dc.getmysql();
						String sqlserver = "SELECT * FROM command_info WHERE device_id="+ 1 + " AND command_id=" + (send%9+1);
						System.out.println("sqlserver " + sqlserver);
						Statement statement = conn.createStatement();
						ResultSet rs = statement.executeQuery(sqlserver);
						if(rs.next()){
							command_length = rs.getInt("length");

							byte[] sendBuffer = new byte[command_length+3];
							//sendBuffer[0] = (byte)("0x"+rs.getString("flag1"))
							sendBuffer[0] = (byte)0xef;
							sendBuffer[1] = (byte)0xef;
							sendBuffer[2] = rs.getByte("length");
							sendBuffer[3] = (byte)(rs.getInt("address"));
							sendBuffer[4] = rs.getByte("command");
							sendBuffer[5] = (byte)(sendBuffer[0]+sendBuffer[1]+sendBuffer[2]+sendBuffer[3]+sendBuffer[4]);
